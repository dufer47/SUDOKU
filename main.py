...
# Funcion para mostrar la pantalla de ingreso de nombre
        ...
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                if bandera_jugar == True:
                    ...
                    pygame.quit()   # Cierra los modulos inicializados
                    exit()  #   Fin ejecucion
                else:
                    pygame.quit()
                    exit()
                ...
                if evento.key == pygame.K_RETURN:  # confirmar con enter
                    if nombre.strip():  # asegurar que no este vacio. Elimina los espacios en blanco en inicio y fin de la cadena
                        ingresando = False
                elif evento.key == pygame.K_BACKSPACE:  # borrar ultimo caracter. Detecta Backspace
                    nombre = nombre[:-1]    # Borra el ultimo caracter
                else:
...
# Funcion para mostrar la pantalla de seleccion de dificultad
...
def mostrar_mensaje_derrota(ventana:pygame.Surface, puntaje:int)->str:
    ...
    while corriendo:
        ...
        texto_instrucciones = fuente_puntaje.render("Presiona ESC para regresar al menu", True, COLOR_BLANCO)
...
def mostrar_mensaje_victoria(ventana:pygame.Surface, puntaje:int)->str:
    ...
    for _ in range(cantidad_numeros):
        ...
        y = random.randint(ALTO_VENTANA + 10, ALTO_VENTANA + 500)  # Inicializar mas abajo
        posiciones.append((x, y))
...
# Funcion para mostrar la pantalla de puntajes
def mostrar_puntajes(ventana: pygame.Surface)->None:
    ...
    boton_menu = pygame.Rect(223, 650, 200, 50)  # Rect del boton "Volver al menu"
    ...
    while en_pantalla_puntajes:
        ...
        y_pos = 280  # Posicion inicial para mostrar puntajes
        for entrada in mejores_puntajes:
            ...
            y_pos += 40  # Separacion entre lineas

        # Dibujar el boton
        pygame.draw.rect(ventana, color_variable, boton_menu)
        texto_boton_menu = fuente.render("Volver al Menu", True, COLOR_NEGRO)
...
def dibujar_tablero(ventana:pygame.Surface, sudoku:list, sudoku_inicial:list, celda_resaltada=None)->None:
    """
    Dibuja el tablero de Sudoku en la ventana principal, incluyendo celdas, numeros y resaltados.
...
while True:
    if estado == MENU:
        # Crear rectangulos interactivos para botones del menu
                                # x    y  ( tamaño (ancho x altura))
        rect_jugar = pygame.Rect(253, 437, 145, 35)  
        rect_puntaje = pygame.Rect(253, 475, 145, 35)  
        rect_salir = pygame.Rect(253, 513, 145, 35)  

        # CREAR LA VENTANA PRINCIPAL
        ventana = pygame.display.set_mode((ANCHO_VENTANA, ALTO_VENTANA))    # Crea ventana con las dimenciones especificadas
        pygame.display.set_caption("Sudoku Interactivo")    # Titulo en la barra superior de la ventana
        ...
        menu_activado = True
        while menu_activado:
            ventana.fill(COLOR_NEGRO)   # Pinta toda la pantalla de negro
            ventana.blit(imagen_fondo_sudoku, (0, 0))   # Dibuja la img sobre la otra superficie
            ...
            for evento in pygame.event.get():
                if evento.type == pygame.QUIT:
                    ...
                    # Calcular puntaje antes de salir
                        tiempo_total = pygame.time.get_ticks() - tiempo_inicial  # Calcular tiempo transcurrido en milisegundos
                        puntaje_final = calcular_puntaje(dificultad, errores, tiempo_total, MULTIPLICADOR_PENA_SEGUN_DIFICULTAD)  # Usar la funcion calcular_puntaje
...
# EVENTOS (juego) --------------------------------------------------------------------------------------------------------------------------------
        ...
        # Verificar si el tablero esta completo despues de procesar eventos
            ...
            pygame.draw.rect(ventana, color_boton_nuevo_tablero, rect_nuevo_tablero, border_radius=10)  # Fondo del boton
            pygame.draw.rect(ventana, color_borde_nuevo_tablero, rect_nuevo_tablero, width=3, border_radius=10)  # Borde del boton
